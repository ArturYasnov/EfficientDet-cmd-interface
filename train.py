...
def run_pl_main(hparams):
    ...
    if hparams.weights:
        model = LitAutoEncoder(image_size=hparams.img,
                               batch_size=hparams.batch_size,
                               lr=hparams.lr,
                               num_classes=data_conf['nc'],
                               data_train=data_conf['train'],
                               data_valid=data_conf['val']).load_from_checkpoint(hparams.weights)
    else:
        model = LitAutoEncoder(image_size=hparams.img,
                               batch_size=hparams.batch_size,
                               lr=hparams.lr,
    ...
    callbacks=lightning_train_callbacks(swa_lr=hparams.swa_lr, swa_epoch_start=hparams.swa_epoch_start, experiment_name=hparams.name)
